...
- Automatic background model requests (memory extraction/consolidation, sharpshooter extraction/consolidation, enhanced speech rewriting, learn capture, auto-thinking and unexpected-stop classification, branch summaries) now use a provider session identity isolated from the foreground turn — like automatic titles since #10621 — so an overlapping side request no longer advances the foreground provider session and rejects the waiting prompt with `This session advanced while the request was waiting` ([#10865](https://github.com/can1357/oh-my-pi/issues/10865)).
