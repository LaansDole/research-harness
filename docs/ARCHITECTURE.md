...
| launcher | `bin/research` | Loads `~/.research-harness/config.env`, exports `RESEARCH_HARNESS_HOME`/`PAPER_GRAPH_DB`, then `exec omp --append-system-prompt research/mode/system.md` (`bin/research:171-176`); `research doctor` verifies the whole installation (`bin/research:36-157`), including the key-gated Jev first pass, which it reports as an optional capability and never fails on |
