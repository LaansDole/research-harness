...
![omp TUI: a researcher starts a scoping review with one plain sentence and gets a database plan. The typed request reads "I'm starting a scoping review on how LLM agents help with triage in the emergency department. Where do I start?"; the reply reports a project created under ~/.research-harness/projects/ with a precise PCC question, and then a table — one row per database (PubMed/MEDLINE, Embase, Scopus, arXiv) — with why-for-this-question, coverage gaps, access, syntax family, and whether the harness can query that source itself. Under the table: the "no single database is sufficient" rule, the sources deliberately skipped and why, the local corpus noted as read-only, "Saved: searches/00-database-plan.md", and the proposed next step.](assets/research/guided-review-poster.png)

_[Watch the capture ↗](https://laansdole.github.io/research-harness/#guided-review)_ — 10 minutes, unedited, four turns: the question, the database plan, the ready-to-paste search strings, and the searches the harness runs itself.

Nothing above is a script. You type what you want in plain language — *where do I start?* — and the harness carries the method: it frames the question as PCC/PICO and writes `scope.md`, says which databases to search for THIS question and why (from the skill's `references/DATABASES.md`: coverage, gaps, access, MeSH vs Emtree vs field-tag syntax), writes the ready-to-paste string for each one, runs the searches it can run itself (arXiv + OpenAlex, or a local PDF corpus via `RESEARCH_CORPUS_DIR` for fully offline runs), and keeps a per-record review store so every PRISMA count is derived rather than typed. Covidence stays optional — `/export` writes RIS/BibTeX when you want it. `/litreview` is the one-command shortcut through the same pipeline.
