...
//! identity, shared profile, container credentials, then IMDSv2. Secret values
//! cross this module only as [`SecretString`] and every public diagnostic is
//! deliberately source-typed and redacted.
...
pub const AWS_FILE_SESSION_CREDENTIAL_TTL: Duration = Duration::from_secs(5 * 60);
...
impl AwsCredentialError {
	fn credential_error(&self) -> CredentialError {
		match self {
	...
	async fn http(
		...
		let response = if let Some(deadline) = deadline {
			match time::timeout(deadline, exchange).await {
				...
				Err(_) => {
					cancellation.cancel();
					return Err(AwsCredentialError::Transport {
						operation,
						source: OAuthTransportError,
					});
				},
