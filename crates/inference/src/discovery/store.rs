...
pub struct DiscoveryCacheKey {
	...
	/// Opaque credential/account or endpoint affinity. Empty means provider-wide.
...
impl DiscoveryStore {
	...
	pub fn prune_expired(&self, now_ms: u64) -> Result<usize, DiscoveryStoreError> {
		...
		transaction.execute(
			"DELETE FROM discovery_generations WHERE expires_at_ms<=?1",
			[now_ms],
		)?;
		transaction.commit()?;
		Ok(stale.len())
...
mod tests {
	...
	fn empty_generation_replaces_stale_rows_and_explicit_invalidation_removes_it() {
		...
		assert!(store.load_fresh(&key, 201).expect("load after invalidation").is_none());
	}
}
