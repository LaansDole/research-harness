...
async fn serve(args: BrowserRelayArgs) -> miette::Result<()> {
	...
	let relay = match RelayServer::start(RelayOptions {
		...
		Ok(relay) => relay,
		Err(error) if error.is_addr_in_use() && probe_relay_server(&endpoint) => {
			println!("omp browser relay already running on {endpoint}; nothing to do.");
			return Ok(());
		},
	...
	println!(
		"  extension endpoint  ws://127.0.0.1:{}/ext{}",
		args.port,
